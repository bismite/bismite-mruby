...
if /Darwin/ === `uname -a` # macos
  ARGV.each do |command|
    case command
    when "--libs"
      puts "-L#{root}/lib -lmruby -lbismite-core -lbismite-ext -lSDL2 -lSDL2_mixer -lSDL2_image -framework OpenGL"
    when "--static-libs"
  ...
  ARGV.each do |command|
    case command
    when "--libs"
      puts "-L#{root}/lib -lmruby -lbismite-core -lbismite-ext -lm -lGL -ldl"
    when "--cflags"
